...
const LOCAL_STORAGE_KEY = "BusesAndaluciaOriental-color-data";

const BusesAndaluciaOriental: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Oriental`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Oriental:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Oriental`
      );
      ...
      toast.error("Error al cargar los datos de Oriental.");
    }
  };
...
{mostrarHistorial && nFlotaSeleccionado && (
  ...
  >
    ...
    <h2 style={titleStyle}>Oriental</h2>
...
export default BusesAndaluciaOriental
