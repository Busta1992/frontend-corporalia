...
const LOCAL_STORAGE_KEY = "Extremadura-color-data";

const Extremadura: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Extremadura`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Extremadura:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Extremadura`
      );
      ...
      toast.error("Error al cargar los datos de Extremadura.");
    }
  };
...
{mostrarHistorial && nFlotaSeleccionado && (
  ...
  >
    ...
    <h2 style={titleStyle}>Extremadura</h2>
