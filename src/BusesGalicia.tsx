...
const LOCAL_STORAGE_KEY = "Galicia-color-data";
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Galicia`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Galicia :", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Galicia`
      );
      ...
      toast.error("Error al cargar los datos de Galicia.");
    }
  };
...
{mostrarHistorial && nFlotaSeleccionado && (
  ...
  >
    ...
    <h2 style={titleStyle}>Galicia</h2>
