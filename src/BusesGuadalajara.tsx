...
const LOCAL_STORAGE_KEY = "Guadalajara-color-data";
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Guadalajara`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Guadalajara:", error);
    });
}, []);
...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Guadalajara`);
...
const CalendarTable: React.FC<CalendarTableProps> = ({ rows, filters, setFilters, handleInputChange }) => {
  ...
  <div style={containerStyle}>
    ...
    <h2 style={titleStyle}>Guadalajara</h2>
