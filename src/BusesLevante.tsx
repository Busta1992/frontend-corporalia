...
const LOCAL_STORAGE_KEY = "Levante-color-data";

const Levante: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Levante`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Levante:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Levante`
      );
      ...
      toast.error("Error al cargar los datos de Levante.");
    }
  };
...
{mostrarHistorial && nFlotaSeleccionado && (
  ...
  >
    ...
    <h2 style={titleStyle}>Levante</h2>
