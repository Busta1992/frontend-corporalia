...
const LOCAL_STORAGE_KEY = "Murcia-color-data";

const Murcia: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Murcia`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Murcia:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Murcia`
      );
      ...
      toast.error("Error al cargar los datos de Murcia.");
    }
  };
