...
const LOCAL_STORAGE_KEY = "Toledo-color-data";

const Toledo: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Toledo`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Toledo:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Toledo`
      );
      ...
      toast.error("Error al cargar los datos de Toledo.");
    }
  };
