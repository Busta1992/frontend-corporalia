...
const LOCAL_STORAGE_KEY = "Valladolid-color-data";

const Valladolid: React.FC = () => {
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Valladolid`)
    .then((res) => {
      setData(res.data);
      setOriginalData(res.data); // Guardamos una copia original para filtros/reset
    })
    .catch((error) => {
      console.error("Error al obtener datos de Valladolid:", error);
    });
}, []);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/carreteras/carretera/Valladolid`
      );
      ...
      toast.error("Error al cargar los datos de Valladolid.");
    }
  };
...
{mostrarHistorial && nFlotaSeleccionado && (
  ...
  >
    ...
    <h2 style={titleStyle}>Valladolid</h2>
