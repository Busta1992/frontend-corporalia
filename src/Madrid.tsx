...
const LOCAL_STORAGE_KEY = "Madrid-color-data";
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/BBDD/provincia/Madrid`)
    .then((res) => {
      setData(res.data);
...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/BBDD/provincia/Madrid`);
      ...
      const formattedData: Row[] = response.data.map((row: any) => {
        ...
        return {
          ...
          municipio: row.municipio?.trim() || "Madrid",
          provincia: row.provincia || "",
          observaciones: row.observaciones || "",
