...
const LOCAL_STORAGE_KEY = "Malaga-color-data";
...
useEffect(() => {
  axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/BBDD/provincia/Malaga`)
    .then((res) => {
      setData(res.data);
...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/BBDD/provincia/Malaga`);
      ...
      const formattedData: Row[] = response.data.map((row: any) => {
        ...
        return {
          ...
          municipio: row.municipio?.trim() || "Malaga",
          provincia: row.provincia || "",
          observaciones: row.observaciones || "",
      ...
      toast.error("Error al cargar los datos de Malaga.");
    }
  };
