...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/Corporalia/v1/mobiliario/provincia/Guadaira`);
...
const coincideCodigo = filterCodigo === "Todos" || r.codigo === filterCodigo;
  ...
  if (color === "white") {
    return (
      (r.color === "white" || (!r.fecha_inicio && !r.fecha_fin)) &&
      coincideProvincia &&
      coincideCampania &&
      coincideMunicipio&&
      coincideCodigo 
...
<th>Fecha Fin</th>
  ...
  <td key={field}>
    <input
      ...
      value={
        (field === "fecha_inicio" || field === "fecha_fin") && (row as any)[field]
